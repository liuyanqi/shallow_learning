...
from model import auxiliary_classifier
...
def train_layer(all_block, train_block, classifier, data_loader, downsampling=False):
	...
	for e in range(max_epoch):
		if e % 15 == 0:
			lr=lr/5.0
			to_train = list(train_block.parameters()) + list(classifier.parameters())
			optimizer = torch.optim.SGD(to_train, lr=lr, momentum=0.9, weight_decay=5e-4)
			print('new lr:',lr)
		...
		for batch_idx, (inputs, targets) in enumerate(data_loader):
			...
			feature = train_block(inputs, targets)
...
classifier = auxiliary_classifier(256, 256, 16, 32, k=1).cuda()

block1.train()
train_layer(all_block, block1, classifier, trainloader_classifier, downsampling=False)
block1.eval()
...
start_time = time.time()
block2 = ShallowBlock(256, 256, aux_feature=256, avg_size=16, in_size=32).cuda()
classifier = auxiliary_classifier(256, 256, 16, 32, k=1).cuda()

block2.train()
train_layer(all_block, block2, classifier, trainloader_classifier, downsampling=False)
block2.eval()
...
start_time = time.time()
block3 = ShallowBlock(1024, 512, aux_feature=512, avg_size=8, in_size=16).cuda()
classifier = auxiliary_classifier(512, 512, 8, 16, k=1).cuda()

block3.train()
train_layer(all_block, block3, classifier, trainloader_classifier, downsampling=True)
block3.eval()
...
classifier = auxiliary_classifier(1024, 1024, 4, 8, k=1).cuda()


block4.train()
train_layer(all_block, block4, classifier, trainloader_classifier, downsampling=True)
block4.eval()
...
start_time = time.time()
block5 = ShallowBlock(4096, 2048, aux_feature=2048, avg_size=2, in_size=4).cuda()
classifier = auxiliary_classifier(2048, 2048, 2, 4, k=1).cuda()
block5.train()
train_layer(all_block, block5, classifier, trainloader_classifier, downsampling=True)
block5.eval()
